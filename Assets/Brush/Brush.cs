...
public class Brush : MonoBehaviour {
    ...
    // Load the Input Action Asset if it's already added in the project
    [SerializeField] private InputActionAsset inputActionAsset; // Drag your Input Action Asset here
    ...
    // Used for tracking button press
    ...
    {    
        ...
        {
            // Find the action map and action by name
            var leftActionMap = inputActionAsset.FindActionMap("XRI Left Interaction", true);
            leftActivateAction = leftActionMap.FindAction("Activate", true);
            leftActivateAction.Enable(); // Ensure the action is enabled

            var rightActionMap = inputActionAsset.FindActionMap("XRI Right Interaction", true);
            rightActivateAction = rightActionMap.FindAction("Activate", true);
            rightActivateAction.Enable(); // Ensure the action is enabled

            Debug.Log("Action button found!");
        }
        else
        {
    ...
    private void Update() {
        ...
        bool rightTriggerPressed = rightActivateAction != null && rightActivateAction.triggered;
        bool leftTriggerPressed = leftActivateAction != null && leftActivateAction.triggered;
        bool triggerPressed = leftTriggerPressed || rightTriggerPressed;
